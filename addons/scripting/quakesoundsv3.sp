...
char g_sRevengeSound[MAX_NUM_SETS][PLATFORM_MAX_PATH];
...
int g_iRevengeConfig[MAX_NUM_SETS];
...
// Revenge
int g_iLastKillers[MAXPLAYERS+1];
...
public void OnPluginStart() {
	...
	g_cvarRevengeMode = CreateConVar("sm_quakesoundsv3_revenge_mode", "1", "How to determine if a kill is a revenge; 1=Use the game's revenge system(CS:S and CS:GO), 2=Use last killer system.", FCVAR_NONE, true, 0.0);
...
// =============
	...
	// =============
	if (SetFileKV.JumpToKey("revenge")) {
		if (SetFileKV.GotoFirstSubKey()) {
			PrintToServer("[SM] Quake Sounds v3  'revenge' section not configured correctly in %s.", setFile);
			SetFileKV.GoBack();
		}
		else {
			SetFileKV.GetString("sound", g_sRevengeSound[setNum], PLATFORM_MAX_PATH);
			g_iRevengeConfig[setNum] = SetFileKV.GetNum("config", 9);
			Format(bufferString, PLATFORM_MAX_PATH, "sound/%s", g_sRevengeSound[setNum]);

			if (FileExists(bufferString, true)) {
				PrecacheSoundCustom(g_sRevengeSound[setNum], PLATFORM_MAX_PATH);
				AddFileToDownloadsTable(bufferString);
			}
			else {
				g_iRevengeConfig[setNum] = 0;
				PrintToServer("[SM] Quake Sounds v3  File specified in 'revenge' does not exist in '%s', ignoring.", setFile);
			}
		}
	}
	else {
		PrintToServer("[SM] Quake Sounds v3  'revenge' section missing in %s.", setFile);
	}
	...
	if (victimClient < 1 || victimClient > GetMaxHumanPlayers()) {
		...
		if (attackerClient == victimClient || attackerClient == 0) {
			...
			if (g_GameEngine == Engine_CSS || g_GameEngine == Engine_CSGO) {
				...
				if (g_cvarRevengeMode.IntValue == 1) {
					revenge = event.GetBool("revenge");
				}
			}
			...
			if (g_cvarRevengeMode.IntValue == 2 && g_iLastKillers[attackerClient] == victimClient) {
				g_iLastKillers[attackerClient] = 0;
				revenge = true;
			}
			g_iLastKillers[victimClient] = attackerClient;
			...
			for (int i = 1; i <= GetMaxHumanPlayers(); i++) {
				if (IsClientInGame(i) && !IsFakeClient(i) && g_iSoundPreference[i] >= 0) {
					if (revenge && g_iRevengeConfig[g_iSoundPreference[i]] > 0) {
						int config = g_iRevengeConfig[g_iSoundPreference[i]];
						char soundFile[PLATFORM_MAX_PATH];
						soundFile = g_sRevengeSound[g_iSoundPreference[i]];
						if (soundFile[0] != '\0') {
							if (config & 1) {
								EmitSoundToClient(i, soundFile, .volume = g_cvarVolume.FloatValue);
							}
							else if ((config & 2) && attackerClient == i) {
								EmitSoundToClient(i, soundFile, .volume = g_cvarVolume.FloatValue);
							}
						...
						if (g_iTextPreference[i]) {
							if (config & 8) {
								PrintCenterText(i, "%t", "revenge", attackerName, victimName);
							}
							else if ((config & 16) && attackerClient == i) {
								PrintCenterText(i, "%t", "revenge", attackerName, victimName);
							}
					...
					else if (firstblood && g_iFirstbloodConfig[g_iSoundPreference[i]] > 0) {
...
public void EventPlayerDisconnect(Event event, const char[] name, bool dontBroadcast) {
	RevengeGarbageCollect(GetClientOfUserId(event.GetInt("userid")));
}

void RevengeGarbageCollect(int playerClient) {
	g_iLastKillers[playerClient] = 0;

	for (int i = 1; i <= MAXPLAYERS; i++) {
		if (g_iLastKillers[i] == playerClient) {
			g_iLastKillers[i] = 0;
		}
	}
}
